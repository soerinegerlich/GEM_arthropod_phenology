...
for (i in unique(df8$SpeciesID)){
  ...
  pdf(paste("Data/Linear_regression_figures\\Figures_lmOnset",i,".pdf"),width=20,height=12)
  #par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
  for (j in unique(df8b$Plot)){
    ...
    if(sum(!is.na(df8a$Onset))<10){ #sum(is.na) finder alle NA værdier. !is.na fjerner alle NA værdier i en vektor. Men denne kan vel ikke bruges her?
      ...
      #pdf(paste("Data/Linear_regression_figures\\Figure_lmOnset",i,".pdf"),width=20,height=12)
      plot(df8a$Year,df8a$Onset,type="p",col="black",main=j,
           ylim=c(154,220), xlim=c(1996,2020)) 
      #title(xlab="Year", ylab="Day of the year")
      points(df8a$Year,df8a$Onset,pch=16,cex=1.5)
      #par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
      #points(df8a$Year,df8a$Onset,pch=16,cex=1.5)
      abline(lm(Onset ~ Year, data =df8a))
...
for (i in unique(df8$SpeciesID)){
  #print(i)
  df8b<-subset(df8,SpeciesID==i)
  pdf(paste("Data/Linear_regression_figures\\Figures_lmPeak",i,".pdf"),width=20,height=12)
  #par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
  for (j in unique(df8b$Plot)){
    df8a<-subset(df8b,Plot==j)
...
if(sum(!is.na(df8a$Peak))<10){
  ...
  #pred$Onset <- df8a$Onset
  #pdf(paste("Data/Linear_regression_figures\\Figure_lmOnset",i,".pdf"),width=20,height=12)
  plot(df8a$Year,df8a$Peak,type="p",col="black",main=j,
       ylim=c(160,240), xlim=c(1996,2020)) 
  #title(xlab="Year", ylab="Day of the year")
  points(df8a$Year,df8a$Peak,pch=16,cex=1.5)
  #par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
  #points(df8a$Year,df8a$Onset,pch=16,cex=1.5)
  abline(lm(Peak ~ Year, data =df8a))
  df_temp<-data.frame(SpeciesID=df8a$SpeciesID[1],
                      Plot=df8a$Plot[1],
                      Pheno_event="Peak",
...
df_summary2<-data.frame(SpeciesID=character(),Plot=character(),Pheno_event=character(),Slope=numeric(),
                        SD=numeric(),Tvalue=numeric(),Pvalue=numeric(),Rsquare=numeric(),AdjRsquare=numeric(),Count=numeric(),n=numeric())

for (i in unique(df8$SpeciesID)){
  #print(i)
  df8b<-subset(df8,SpeciesID==i)
  pdf(paste("Data/Linear_regression_figures\\Figures_lmEnd",i,".pdf"),width=20,height=12)
  #par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
  for (j in unique(df8b$Plot)){
    df8a<-subset(df8b,Plot==j)
...
if(sum(!is.na(df8a$End))<10){
  ...
  #pred$Onset <- df8a$Onset
  #pdf(paste("Data/Linear_regression_figures\\Figure_lmOnset",i,".pdf"),width=20,height=12)
  plot(df8a$Year,df8a$End,type="p",col="black",main=j,
       ylim=c(175,240), xlim=c(1996,2020)) 
  #title(xlab="Year", ylab="Day of the year")
  points(df8a$Year,df8a$End,pch=16,cex=1.5)
  #par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
  #points(df8a$Year,df8a$Onset,pch=16,cex=1.5)
  abline(lm(End ~ Year, data =df8a))
  df_temp<-data.frame(SpeciesID=df8a$SpeciesID[1],
                      Plot=df8a$Plot[1],
                      Pheno_event="End",
  ...
  df_summary2<-bind_rows(df_summary2,df_temp)
    }
    #plot(mod2)
  }
  par(mfrow=c(1,1),oma = c(5,4,4,2),mar = c(2,2,2,2) + 0.1)
