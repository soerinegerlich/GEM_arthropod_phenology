...
for (i in unique(df_phen_event_air$SpeciesID)){
  ...
  for (j in unique(df8b$Plot)){
    ...
    if(length(df8sub$Year)<5){ #sum(is.na) finder alle NA værdier. !is.na fjerner alle NA værdier i en vektor. Men denne kan vel ikke bruges her?
      ...
      mod1 <- lm(Onset ~ Snowmelt+Onset_Temp+Year, data=df8sub)       
      AIC <- AIC(mod1)
      ...
      df_temp<-data.frame(SpeciesID=df8sub$SpeciesID[1], 
                          ...
                          SE1=summary(mod1)$coefficients[6],
                          SE2=summary(mod1)$coefficients[7],
                          Tvalue1=summary(mod1)$coefficients[10],
                          Tvalue2=summary(mod1)$coefficients[11],
                          Pvalue1=summary(mod1)$coefficients[14],
                          Pvalue2=summary(mod1)$coefficients[15],
    ...
    if(length(df8sub$Year)<5){
      ...
      mod2 <- lm(Peak ~ Snowmelt+Peak_Temp+Year, data=df8sub)
      AIC2 <- AIC(mod1)
      ...
      df_temp<-data.frame(SpeciesID=df8sub$SpeciesID[1],
                          ...
                          SE1=summary(mod2)$coefficients[6],
                          SE2=summary(mod2)$coefficients[7],
                          Tvalue1=summary(mod2)$coefficients[10],
                          Tvalue2=summary(mod2)$coefficients[11],
                          Pvalue1=summary(mod2)$coefficients[14],
                          Pvalue2=summary(mod2)$coefficients[15],
                          Count=sum(df8sub$TotalAbundance),
    ...
    if(length(df8sub$Year)<5){
      ...
      mod3 <- lm(End ~ Snowmelt+End_Temp+Year, data=df8sub)
      AIC3 <- AIC(mod3)
      ...
      df_temp<-data.frame(SpeciesID=df8sub$SpeciesID[1],
                          ...
                          SE1=summary(mod3)$coefficients[6],
                          SE2=summary(mod3)$coefficients[7],
                          Tvalue1=summary(mod3)$coefficients[10],
                          Tvalue2=summary(mod3)$coefficients[11],
                          Pvalue1=summary(mod3)$coefficients[14],
                          Pvalue2=summary(mod3)$coefficients[15],
                          Count=sum(df8sub$TotalAbundance),
    ...
    if(length(df8sub$Year)<5){
      ...
      mod4 <- lm(Duration ~ Snowmelt+Onset_Temp+Year, data=df8sub)
      AIC4 <- AIC(mod4)
      ...
      df_temp<-data.frame(SpeciesID=df8sub$SpeciesID[1],
                          ...
                          SE1=summary(mod4)$coefficients[6],
                          SE2=summary(mod4)$coefficients[7],
                          Tvalue1=summary(mod4)$coefficients[10],
                          Tvalue2=summary(mod4)$coefficients[11],
                          Pvalue1=summary(mod4)$coefficients[14],
                          Pvalue2=summary(mod4)$coefficients[15],
                          Count=sum(df8sub$TotalAbundance),
...
#write_xlsx(df_summary_all_air, "Data/Climate_pheno\\df_summary_all_air_detrend.xlsx", col_names = TRUE)

#mean(df_summary_all_air$vif_snow)
...
for (i in unique(df_phen_event_air$SpeciesID)){
  ...
  for (j in unique(df8b$Plot)){
    ...
    if(length(df8sub$Year)<5){ #sum(is.na) finder alle NA værdier. !is.na fjerner alle NA værdier i en vektor. Men denne kan vel ikke bruges her?
      ...
      mod1 <- lm(Onset ~ Onset_Temp+Snowmelt+Year, data=df8sub)       
      AIC <- AIC(mod1)
      R.mod1 <- rsq(mod1, adj = TRUE)
      Residual1 <- sqrt(deviance(mod1)/df.residual(mod1))
      CI <- confint(mod1, level=0.95)
    ...
    if(length(df8sub$Year)<5){
      ...
      mod2 <- lm(Peak ~ Peak_Temp+Snowmelt+Year, data=df8sub)
      AIC2 <- AIC(mod1)
      R.mod2 <- rsq(mod2, adj = TRUE)
      Residual2 <- sqrt(deviance(mod2)/df.residual(mod2))
      CI2 <- confint(mod2, level=0.95)
    ...
    if(length(df8sub$Year)<5){
      ...
      mod3 <- lm(End ~ End_Temp+Snowmelt+Year, data=df8sub)
      AIC3 <- AIC(mod3)
      R.mod3 <- rsq(mod3, adj = TRUE)
      Residual3 <- sqrt(deviance(mod3)/df.residual(mod3))
      CI3 <- confint(mod3, level=0.95)
    ...
    if(length(df8sub$Year)<5){
      ...
      mod4 <- lm(Duration ~ Onset_Temp+Snowmelt+Year, data=df8sub)
      AIC4 <- AIC(mod4)
      R.mod4 <- rsq(mod4, adj = TRUE)
      Residual4 <- sqrt(deviance(mod4)/df.residual(mod4))
      CI4 <- confint(mod4, level=0.95)
...
Onset_temp <- ggplot(df_Onset, aes(x = Slope1, y = SpeciesID, xmin = CI_lwr1, xmax = CI_upr1, color = cut(Pvalue1, breaks = c(0, 0.05, 0.099, 1))))+
  xlab("")+
  ylab("")+
  #scale_shape(guide = "none")+
...
Peak_temp <- ggplot(df_Peak, aes(x = Slope1, y = SpeciesID, xmin = CI_lwr1, xmax = CI_upr1, color = cut(Pvalue1, breaks = c(0, 0.05, 0.099, 1))))+
  xlab("")+
  ylab("")+
  #xlim(c(-15,10))+
  #scale_colour_discrete(name = "Taxon")+
...
End_temp <- ggplot(df_End, aes(x = Slope1, y = SpeciesID, xmin = CI_lwr1, xmax = CI_upr1, color = cut(Pvalue1, breaks = c(0, 0.05, 0.099, 1))))+
...
Duration_temp <- ggplot(df_Duration, aes(x = Slope1, y = SpeciesID, xmin = CI_lwr1, xmax = CI_upr1, color = cut(Pvalue1, breaks = c(0, 0.05, 0.099, 1))))+
  xlab("Phenological sensitivity (days per °C)")+
  ylab("")+
  #xlim(c(-15,10))+
  #scale_colour_discrete(name = "Taxon")+
